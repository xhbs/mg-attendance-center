 ...
 * @lastTime: 2021-02-04 09:08:01
 * @文件相对于项目的路径: \biz-scene-attendancesys-fe\src\components\AttendDetailModal\index.jsx
 * @Description: 考勤明细详情弹框  日常
 */
...
import { getDailyDetail } from '@/services/detailPage';
...
const AttendDetailModal = (props) => {
    const { visible, onCancel, dataArr, currentIndex } = props;
    ...
    const getDetail = (detail) => {
        ...
        getDailyDetail(params).then(res => {
            if (res && res.error_code === API_ERROR_CODE) {
                const { person = {}, info = {}, leave_info, practice_info, recent_records = [], status_changed_record = [] } = res.data;
                // 基本信息
                person.admission_time = person.admission_time && moment(person.admission_time).format('YYYY-MM-DD') || '';

                // 考勤信息  校级的只有考勤日期
                info.ats_day = info.ats_day && moment(info.ats_day).format('YYYY-MM-DD') || '';
                info.class_time = info.class_time && moment(info.class_time).format('YYYY-MM-DD HH:mm:ss') || '';
                ...
                // 比对时的照片
                let recordArr = [...status_changed_record];
                if(status_changed_record.length > 0) {
                    recordArr = recordArr.map(item => {
                        if(item.misreport_info) {
                            item.misreport_info.person_url = person.person_url || '';
                        }
                        return item;
                    })
                }
                ...
                setDetailObj({
                    ...
                    practice_info,
                    recent_records,
                    status_changed_record: recordArr,
                    personUrl: person.person_url,
                });
            }
        }).finally(() => {
            setLoading(false);
                            ...
                            <ul style={{marginTop: '10px'}}>
                                {
                                    ...
                                    ? renderTextList('list', dataLabel.attendance_info_daily_noTime, detailObj.attendance_info)
                                    : renderTextList('list', dataLabel.attendance_info_daily, detailObj.attendance_info)
                                }
                                {/* 缺勤、请假、实习状态显示无考勤记录 */}
                                {
                                    (!status || ['缺勤', '请假', '实习'].includes(status)) && (
...
AttendDetailModal.defaultProps ={
    visible: false,
    onCancel: () => {},
...
export default AttendDetailModal;